...
class Player {
	...
	public int turn() {
		...
		System.out.print("6초 예상 후 <Enter>키>>");
		// <Enter> 키 때가지 기다림
	...
	public void run() {
		...
		player[0] = new Player("홍자바");
		player[1] = new Player("김자바");
	
		System.out.println("6초에 가까운 사람이 이기는 게임입니다.");
		...
		if(Math.abs(6-duration1) < Math.abs(6-duration2)) // 시간 간격이 작은 사람이 이김
